...
# First observation some movies get rated more than others, here is the distribution:
...
## First model: we predict the same rating for all movies regardless of user
...
# the lm() function will be very slow here because there is bias, each movie gets one
...
## Third model: User reaction
# Let’s compute the average rating for user μ for those that have rated over 100 movies.
...
## Results: The third model has the lowest RMSE and will be used for final testing of the validation set
rmse_results %>% knitr::kable()
